...
#'
#'   uk_lms <- twostep(tpb_uk, TPB_UK, method = "lms", nodes = 32, adaptive = "quasi")
#'   summary(uk_lms)
#' }
#' @export
twostep <- function(model.syntax, data, method = "lms", ...) {
  ...
  else mod_msg_stop(paste0("Unsupported method: ", method))
...
twostepDA <- function(model.syntax, data, method = "lms", zero.tol = 1e-12,
                      fix.cov.xis = TRUE, ...) {
  mod_stopif(!method %in% DA_METHODS, paste0("Unsupported method: ", method))
