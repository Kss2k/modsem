...
If you do it this way, there will however be no correction to the product indicator 
when using one of the product indicator approaches.
To fix this we could instead apply the correction to the generated syntax.

```{r}
library(lavaan)

tpb_uk_pi <- get_pi_syntax(tpb_uk, method = "dblcent", 
                           res.cov.method = "none") # residual covariances are irrelevant
TPB_UK_pi <- get_pi_data(tpb_uk, data = TPB_UK, method = "dblcent")

corrected <- relcorr_single_item(syntax = tpb_uk_pi, data = TPB_UK_pi)
syntax <- corrected$syntax
data   <- corrected$data

est_dca <- sem(syntax, data = data)
summary(est_dca)
```

The easiest approach howerver, is to use the `rcs` argument in the `modsem` function
...
```{r}
relcorr_single_item(syntax = tpb_uk_pi, data = TPB_UK_pi, 
                    choose = c("ATT", "SN", "PBC", "INT", "INTPBC"))
