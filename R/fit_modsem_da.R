...
fit_modsem_da <- function(model, chisq = TRUE, lav.fit = FALSE,
  ...
  if (isTRUE(chisq) && any(grepl(":", parTable$rhs), na.rm = TRUE)) {
    mod_msg_warn("Chi-Square based fit-measures for LMS and QML ",
  ...
  for (g in seq_len(n.groups)) {
    ...
    if (chisq && !is.null(E.g)) {
      chi.g <- tryCatch(
        ...
        error = function(e) {
          mod_msg_warn("Failed to compute chi-square contribution for group ",
  ...
  if (lav.fit) {
    lavfit <- tryCatch(
      ...
      error = function(e) {
        mod_msg_warn("Unable to retrieve fit measures for lavaan model!\n",
