...
`modsem` also handles quadratic effects when plotting interaction using the `plot_interaction()`
...
Here we can see a three-dimensional surface plot.

```{r}
plot_surface(x = "X", z = "Z", y = "Y", model = est3, vals_z = c(-1, 0, 1))
```
