...
gradientLogLikQml <- function(theta, model, epsilon = 1e-8, sign = -1, .f = logLikQml, ...) {
  params <- model$params
  ...
  grad <- matrix(0, n = k, ncol = n, dimnames = list(names(theta), NULL))
  ...
  if (n == 1L) as.vector(grad) else t(grad)
}
